...
players <- data.frame(
  name = c("Valerie Norwood", "Aleshia Jones", "Gracy Wernli", "Ellie Brueggemann",
           "Aalayah Wilson", "Konstantina Mantziori", "Jovana Nikolic", "Justice Macon",
           "Brooke Coffey", "Alyssa Nielsen", "Anna McCarn", "Mya Skoff",
           "Noah Garcia", "Gracie Kelsey"),
  number = c(  1, 2, 3, 4,
               5, 7, 8, 10,
               12, 15, 21, 22,
               27, 33),
      ...
      dashboardPage(
        ...
        dashboardBody(
          tabItems(
            ...
            tabItem(
              tabName = "LindenwoodShotAnalysis",
              fluidPage(
                ...
                fluidRow(
                  ...
                  # Shot type + conditional UI
                  column(
                    6,
                    selectInput("shotType", "Shot Type:", choices = NULL),
                    
                    # Shown only when Dribble is selected
                    uiOutput("dribbleDirectionUI")
                  )
                ),
...
server <- function(input, output, session) {
  ...
  ")
    ...
    "
      ...
      showNotification(paste0("DB ERROR: ", e$message), type = "error", duration = 6)
      message("DATABASE ERROR: ", e$message)  # prints full error in R console
    }, finally = {
    })
  })
  ...
  SELECT event_person AS Player, 
    ...
    ROUND(
        CASE WHEN SUM(CASE WHEN Fg = 1 THEN 1 ELSE 0 END) > 0
        THEN (SUM(CASE WHEN event = 'Shot' AND made_miss = 'Make' THEN 1 ELSE 0 END) * 100.0) / 
             NULLIF(SUM(CASE WHEN Fg = 1 THEN 1 ELSE 0 END),0) 
        ELSE 0 END, 2
    ...
    ROUND(
        CASE WHEN SUM(CASE WHEN Three_Pt = 1 THEN 1 ELSE 0 END) > 0
        THEN (SUM(CASE WHEN Three_Pt = 1 AND made_miss = 'Make' THEN 1 ELSE 0 END) * 100.0) / 
             NULLIF(SUM(CASE WHEN Three_Pt = 1 THEN 1 ELSE 0 END),0)
        ELSE 0 END, 2
    ) AS ThreePtPercentage,

    -- Shots off dribble percentages
    ROUND(
        CASE WHEN SUM(CASE WHEN event = 'Shot' AND dribble_catch = 'Dribble' THEN 1 ELSE 0 END) > 0
        THEN SUM(CASE WHEN event = 'Shot' AND dribble_catch = 'Dribble' AND made_miss = 'Make' THEN 1 ELSE 0 END) * 100.0 / 
             NULLIF(SUM(CASE WHEN event = 'Shot' AND dribble_catch = 'Dribble' THEN 1 ELSE 0 END),0)
        ELSE 0 END, 2
    ) AS ShotsOffDribblePercentage,

    -- Shots off dribble right
    ROUND(
        CASE WHEN SUM(CASE WHEN event = 'Shot' AND dribble_catch = 'Dribble' AND COALESCE(dribble_direction,'') = 'Right' THEN 1 ELSE 0 END) > 0
        THEN SUM(CASE WHEN event = 'Shot' AND dribble_catch = 'Dribble' AND COALESCE(dribble_direction,'') = 'Right' AND made_miss = 'Make' THEN 1 ELSE 0 END) * 100.0 /
             NULLIF(SUM(CASE WHEN event = 'Shot' AND dribble_catch = 'Dribble' AND COALESCE(dribble_direction,'') = 'Right' THEN 1 ELSE 0 END),0)
        ELSE 0 END, 2
    ) AS ShotsOffDribbleRightPercentage,

    -- Shots off dribble left
    ROUND(
        CASE WHEN SUM(CASE WHEN event = 'Shot' AND dribble_catch = 'Dribble' AND COALESCE(dribble_direction,'') = 'Left' THEN 1 ELSE 0 END) > 0
        THEN SUM(CASE WHEN event = 'Shot' AND dribble_catch = 'Dribble' AND COALESCE(dribble_direction,'') = 'Left' AND made_miss = 'Make' THEN 1 ELSE 0 END) * 100.0 /
             NULLIF(SUM(CASE WHEN event = 'Shot' AND dribble_catch = 'Dribble' AND COALESCE(dribble_direction,'') = 'Left' THEN 1 ELSE 0 END),0)
        ELSE 0 END, 2
    ) AS ShotsOffDribbleLeftPercentage,

    -- Shots off catch
    ROUND(
        CASE WHEN SUM(CASE WHEN event = 'Shot' AND dribble_catch = 'Catch' THEN 1 ELSE 0 END) > 0
        THEN SUM(CASE WHEN event = 'Shot' AND dribble_catch = 'Catch' AND made_miss = 'Make' THEN 1 ELSE 0 END) * 100.0 /
             NULLIF(SUM(CASE WHEN event = 'Shot' AND dribble_catch = 'Catch' THEN 1 ELSE 0 END),0)
        ELSE 0 END, 2
    ) AS ShotsOffCatchPercentage,

    -- Shots after rebound
    SUM(CASE WHEN event = 'Shot after rebound' THEN 1 ELSE 0 END) AS ShotsAfterReboundAttempts,
    ROUND(
        CASE WHEN SUM(CASE WHEN event = 'Shot after rebound' THEN 1 ELSE 0 END) > 0
        THEN SUM(CASE WHEN event = 'Shot after rebound' AND made_miss = 'Make' THEN 1 ELSE 0 END) * 100.0 /
             NULLIF(SUM(CASE WHEN event = 'Shot after rebound' THEN 1 ELSE 0 END),0)
        ELSE 0 END, 2
    ) AS ShotsAfterReboundPercentage,
    ...
    -- Free throws
    ROUND(
        CASE WHEN SUM(CASE WHEN Free_Throw = 1 THEN 1 END) > 0
        THEN SUM(CASE WHEN Free_Throw = 1 AND made_miss = 'Make' THEN 1 ELSE 0 END) * 100.0 /
             NULLIF(SUM(CASE WHEN Free_Throw = 1 THEN 1 END),0)
        ELSE 0 END, 2
    ) AS FreeThrowPercentage
...
")
  ...
  unique_games <- reactive({
    ...
    query <- "SELECT DISTINCT game_date, opponent AS opponent_team 
            FROM w_basketball_game_chart_t 
            ORDER BY game_date"
  ...
  observe({
    ...
    if (nrow(game_data) > 0) {
      updateSelectInput(session, "selected_game",
                        choices = c("All Games", game_data$game_label),
                        selected = "All Games")
    }
  })
  ...
  unique_shot <- reactive({
    ...
    query <- "
    SELECT DISTINCT dribble_catch, event
    FROM w_basketball_game_chart_t
  ...
  "
    ...
    # Build dropdown list
    shot_types <- c("All Shots")
    
    if ("Dribble" %in% shot_df$dribble_catch)
      shot_types <- c(shot_types, "Dribble")
    
    if ("Catch" %in% shot_df$dribble_catch)
      shot_types <- c(shot_types, "Catch")
    
    # Always include this option if database contains it
    if ("Shot After Rebound" %in% shot_df$event)
      shot_types <- c(shot_types, "Shot After Rebound")
  ...
  observe({
    updateSelectInput(session, "shotType",
                      choices = unique_shot(),
                      selected = "All Shots")
  })
  ...
  get_shot_data <- reactive({
    ...
    selected_game_date <- 
      if (input$selected_game != "All Games")
        as.Date(strsplit(input$selected_game, " - ")[[1]][1])
    else NULL
    ...
    query <- "
    SELECT event_person AS number, event, event_x, event_y, event_distance,
           made_miss, game_date, dribble_catch, dribble_direction
    FROM w_basketball_game_chart_t
    WHERE event IN ('Shot', 'Shot After Rebound')
  "
    ...
    } else if (shot_type == "Shot After Rebound") {
      df_shots <- df_shots %>% filter(event == "Shot After Rebound")
    }
  ...
  output$shootingStatsTable <- renderDT({
    ...
    datatable(
      ...
      colnames = c(
        ...
        "Shot Off Dribble Right%", "Shot Off Dribble Left%",
        "Shot After Rebound%"
      )
    )
  })
